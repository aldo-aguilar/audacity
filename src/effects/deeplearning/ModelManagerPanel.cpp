...
ModelManagerPanel::ModelManagerPanel(wxWindow *parent,
...
void ModelManagerPanel::PopulateOrExchange(ShuttleGui &S) {
  S.StartVerticalLay(true);
  {
    ...
    mScroller = S.Style(wxVSCROLL | wxTAB_TRAVERSAL).StartScroller();
    {}
    S.EndScroller();
...
void ModelManagerPanel::AddCard(ModelCardHolder card) {
  ...
  mPanels[repoId] =
      std::make_unique<ModelCardPanel>(mScroller, wxID_ANY, card, mEffect);
...
CardFetchedCallback ModelManagerPanel::GetCardFetchedCallback() {
  CardFetchedCallback onCardFetched = [this](bool success,
                                             ModelCardHolder card) {
    this->CallAfter([this, success, card]() {
      if (success) {
        bool found = mPanels.find(card->GetRepoID()) != mPanels.end();
        bool effectTypeMatches =
            card->effect_type() == mEffect->GetDeepEffectID();
...
void ModelManagerPanel::FetchCards() {
  ...
  CardFetchProgressCallback onCardFetchedProgress = [this](int64_t current,
                                                           int64_t total) {
    this->CallAfter([this, current, total]() {
      if (mTools)
...
ModelCardPanel::ModelCardPanel(wxWindow *parent, wxWindowID winid,
...
bool ModelCardPanel::TransferDataToWindow() { return true; }
...
void ModelCardPanel::PopulateNameAndAuthor(ShuttleGui &S) {
  ...
  {
    mModelName =
        S.AddVariableText(XO("%s").Format(mCard->name()), false, wxLEFT);
  ...
  S.StartHorizontalLay(wxALIGN_LEFT, true);
  {
    ...
    mModelAuthor = S.AddVariableText(XO("%s").Format(mCard->author()));
...
void ModelCardPanel::PopulateDescription(ShuttleGui &S) {
  ...
  mModelDescription = S.AddVariableText(
      XO("%s").Format(wxString(mCard->short_description())), false, wxLEFT);
  S.EndStatic();
}
...
void ModelCardPanel::PopulateMetadata(ShuttleGui &S) {
  ...
  {
    S.AddVariableText(XO("Effect: "))->SetFont(wxFont(wxFontInfo().Bold()));
    S.AddVariableText(XO("%s").Format(mCard->effect_type()));

    S.AddVariableText(XO("Domain: "))->SetFont(wxFont(wxFontInfo().Bold()));
    S.AddVariableText(XO("%s").Format(" NONE ")); // FIXME

    S.AddVariableText(XO("Sample Rate: "))
        ->SetFont(wxFont(wxFontInfo().Bold()));
    S.AddVariableText(XO("%d").Format(mCard->sample_rate()));
  }
  S.EndMultiColumn();
}
...
void ModelCardPanel::PopulateInstallCtrls(ShuttleGui &S) {
  ...
  S.StartVerticalLay(wxCENTER, true);
  {
    ...
    mInstallProgressGauge =
        safenew wxGauge(S.GetParent(), wxID_ANY, 100); // TODO:  sizing
    ...
    S.StartHorizontalLay(wxCENTER, true);
    {
      ...
      TranslatableString status =
          installed ? XO("installed") : XO("uninstalled");
      ...
      SetInstallStatus(installed ? InstallStatus::installed
    ...
    mEnableButton->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
                           wxCommandEventHandler(ModelCardPanel::OnEnable),
                           NULL, this);
...
void ModelCardPanel::OnInstall(wxCommandEvent &event) {
  ...
  // should the destructor of something make sure that no installation was left
  // halfway thru? what does the network manager do in that case?
  ProgressCallback onProgress(
      [this, &manager](int64_t current, int64_t expected) {
        this->CallAfter([current, expected, this, &manager]() {
          if (expected > 0) {
  ...
  CompletionHandler onInstallDone(
      [this, &manager](int httpCode, std::string responseBody) {
        this->CallAfter([this, httpCode, &manager]() {
          if (httpCode == 200 || httpCode == 302) {
            ...
            if (manager.IsInstalled(this->mCard))
              ...
              this->mEffect->Effect::MessageBox(
                  XO("An error ocurred while installing the model with Repo ID "
                     "%s. ")
            ...
            this->SetInstallStatus(InstallStatus::uninstalled);
            this->mEffect->Effect::MessageBox(
                XO("An error ocurred while downloading the model with Repo ID "
                   "%s. \n"
                   "HTTP Code: %d")
                    .Format(this->mCard->GetRepoID(), httpCode));
          }
        });
      });
...
void ModelCardPanel::OnEnable(wxCommandEvent &event) {
  ...
  try {
    ...
    throw SimpleMessageBoxException{
        ...
        XO("An error occurred while trying to load the model. There "
           "may be an issue with the model file. If this problem "
           "persists, try to uninstall and reinstall the model."),
        XO("Warning"), 
        "Error:_ModelException"};
  }
}
